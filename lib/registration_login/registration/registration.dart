...
import 'package:flutter_demo1/registration_login/registration/registration_controller.dart';

class Registration_home extends StatelessWidget {
              ...
              ElevatedButton(
                  onPressed: () {
                    if (_formKey.currentState!.validate()) {
                      if (controller.passTXT.text==controller.conpassTXT.text) {
                        controller.register(context: context);
                      } else {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(elevation: 16,behavior: SnackBarBehavior.floating,content: Text('password does not match')),
                        );
                      }
                    }
                  },
